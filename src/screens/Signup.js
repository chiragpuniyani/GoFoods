...
export default function Signup() {
  ...
  //     const response = await fetch('http://localhost:5000/api/Routes/getlocation', {
  //       method: 'POST',
  //       headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch("https://gofoods-backend.onrender.com/api/createuser', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (json.Success) { // Assuming the server returns { Success: true } on success
